...
const container = document.querySelector(".webgl"),
	slides = document.querySelectorAll(".js-hero-text-slides .hero-slide"), scene = new THREE.Scene,
	gltfLoader = new GLTFLoader, frustumSize = 534, uvsArray = new Float32Array([1, 1, 0, 1, 0, 0, 1, 0]),
	objectUrl = "https://web-assets.chaingpt.org/assets/3d/chainGPT_robo_BAKED_CYCLE.glb",
	envMapUrl = "https://web-assets.chaingpt.org/assets/3d/Cannon_Exterior.hdr", buttonClickDelay = 6.5,
	spherical = new THREE.Spherical, sphericalDelta = new THREE.Spherical, rotateStart = new THREE.Vector2,
	rotateEnd = new THREE.Vector2, rotateDelta = new THREE.Vector2, headInitialRotation = new THREE.Vector3,
	moveThreshold = 30, targetVector = new THREE.Vector3(0, 420, 0), screens = ["hero", "products"], positioning = {
		hero: {
			camera: new THREE.Vector3(462, 740, 612),
			zoom: 1,
			targetVector: new THREE.Vector3(0, 420, 0)
		}, products: {camera: new THREE.Vector3(-38, 560, 612), zoom: 1.4, targetVector: new THREE.Vector3(0, 570, 0)}
...
const clickVideo = document.getElementById("click-video"), clickTexture = new THREE.VideoTexture(clickVideo);
...
const initialPos = camera.position.clone(), initialRot = camera.rotation.clone(),
	...
	i.geometry.setAttribute("uv", new THREE.BufferAttribute(uvsArray, 2)), i.material = shaderMaterial.clone(), i.material.uniforms = {
		...
		document.querySelectorAll("[anim-trigger-hero]").forEach((e => {
			...
			t.to(e.querySelector("[anim-trigger-text-hero]"), {
				...
				trigger: e, start: "top 95%", end: "top center", onEnter: () => {
					setTimeout((() => {
		...
		t.to(e.querySelector("[anim-trigger-text]"), {
			...
			trigger: e, start: "top 92%", onEnter: () => {
				setTimeout((() => {
					e.classList.add("is-animated"), t.timeScale(1).play()
				}), 300)
			}
...
const renderer = new THREE.WebGLRenderer({alpha: !0, antialias: !1});
...
const clock = new THREE.Clock, tick = () => {
...
const getMouseDegrees = (e, t, i) => {
	let r, a, o, n, s = 0, c = 0;
	const l = window.innerWidth, d = window.innerHeight;
	return e <= l / 2 && (r = l / 2 - e, a = r / (l / 2) * 100, s = i * a / 100 * -1), e >= l / 2 && (r = e - l / 2, a = r / (l / 2) * 100, s = i * a / 100), t <= d / 2 && (o = d / 2 - t, n = o / (d / 2) * 100, c = .5 * i * n / 100 * -1), t >= d / 2 && (o = t - d / 2, n = o / (d / 2) * 100, c = i * n / 100), {
		x: s,
		y: c
	}
};
gsap.registerPlugin(ScrollTrigger), ScrollTrigger.create({
	...
	trigger: ".unlimited-solutions", start: "top 10%", onEnter: () => {
	...
	trigger: ".reviews-section", start: "top 70%", onEnter: () => {
		isTestimonials = !0, eyeVideo.currentTime = 2.5, eyeVideo.play(), eyeVideo.removeAttribute("loop")
	}, onLeaveBack: () => {
		isProducts = !0, isTestimonials = !1, eyeVideo.currentTime = 0, eyeVideo.play(), eyeVideo.setAttribute("loop", "true")
	}
});
const changeSlide = () => {
	index = index > slides.length - 2 ? 0 : index, slides.forEach(((e, t) => {
		index === t ? e.classList.add("active") : e.classList.remove("active")
	})), index++
};
changeSlide();
const cameraAnimation = e => {
	...
	gsap.to(camera, {zoom: positioning[screens[0]].zoom + e * (positioning[screens[1]].zoom - positioning[screens[0]].zoom)}), gsap.to(camera.position, {
	...
	const e = new THREE.Vector3, t = (new THREE.Quaternion).setFromUnitVectors(camera.up, new THREE.Vector3(0, 1, 0)),
		i = t.clone().invert(), r = camera.position;
...
const updateEyesPos = e => {
	const t = document.querySelector(".pupils"), i = t.getBoundingClientRect();
	if (i.top > window.innerHeight) return;
	const r = (e.clientX - i.left) / 40 + "px", a = (e.clientY - i.top) / 15 + "px";
